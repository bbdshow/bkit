...
func (h *HTTPRequest) SetCookie(cookies []http.Cookie) *HTTPRequest {
	str := make([]string, 0)
	for _, v := range cookies {
		str = append(str, v.String())
	}
	h.req.Header.Add("Cookie", strings.Join(str, ";"))
	return h
}
...
func copyReqBody(iReq *HTTPRequest) error {
	...
	iBody, err := io.ReadAll(iReq.req.Body)
	if err != nil {
		return err
	}

	iReq.forkReqBody = iBody
	iReq.req.Body = io.NopCloser(bytes.NewBuffer(iBody))
